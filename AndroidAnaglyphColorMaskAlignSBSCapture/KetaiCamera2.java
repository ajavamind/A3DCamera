 ...
 * <p>Modified 2025-11-30 Andy Modla </p>
 */

/**
 * The Class KetaiCamera2 allows the processing sketches to access android
 * cameras through an object modeled after the desktop/java processing Camera
 * class.
 *
 */
public class KetaiCamera2 extends PImage {
  ...
  public volatile boolean photoSaved = false;
   ...
   */
  public KetaiCamera2(PApplet pParent, int _width, int _height, int _framesPerSecond) {
  ...
  public void onPermissionResult(boolean granted) {
    if (!granted) {
      if (DEBUG_CAMERA) PApplet.println("User did not grant camera permission.  Camera is disabled.");
    }
  }
  ...
  private void determineObjectIntentions(Object o) {
    try {
      ...
      if (DEBUG_CAMERA) PApplet.println("Found onCameraPreviewEvent ");
    }
  ...
  public void manualSettings() {
    ...
    try {
      ...
      if (DEBUG_CAMERA) PApplet.println("Failed to set parameters to manual." + x.getMessage());
    }
    // if (DEBUG_CAMERA) PApplet.println("KetaiCamera2 manualSettings: "
  ...
  public void autoSettings() {
    ...
    // if (DEBUG_CAMERA) PApplet.println("KetaiCamera2: setting camera settings to auto...");
    Parameters cameraParameters = camera.getParameters();
    if (cameraParameters.isAutoExposureLockSupported())
    ...
    List<String> fModes = cameraParameters.getSupportedFocusModes();
    for (String s : fModes) {
      // if (DEBUG_CAMERA) PApplet.println("FocusMode: " + s);
    ...
    camera.autoFocus(autofocusCB);
      ...
      PApplet.println("RuntimeException autoFocus failed ...");
    }
    // if (DEBUG_CAMERA) PApplet.println("KetaiCamera2 autoSettings: "
    // + camera.getParameters().flatten());
  }
   ...
   * height since photo quality can be better than preview/camera image.
   *
  ...
  public void setCameraID(int _id) {
    boolean isActive = isStarted;
      ...
      if (_id < getAllCameras() && cameraID != _id) {
        if (isActive)
          this.stop();
        cameraID = _id;
      }
  ...
  public boolean start() {
    ...
    try {
      // if (DEBUG_CAMERA) PApplet.println("KetaiCamera2: opening camera...");
      if (camera == null)
      try {
        ...
        // + x.getMessage());
        if (DEBUG_CAMERA) PApplet.println("Failed to open camera for camera ID: " + cameraID + ":" + x.getMessage());
        return false;
      }
      Parameters cameraParameters = camera.getParameters();
      List<Integer> list = cameraParameters.getSupportedPreviewFormats();

      if (DEBUG_CAMERA) PApplet.println("Supported preview modes...");
      for (Integer i : list) {

        if (i == ImageFormat.RGB_565) {
          //if (DEBUG_CAMERA) PApplet.println("RGB Image preview supported!!!!(try better resolutions/fps combos)");
          isRGBPreviewSupported = true;
        }

        if (DEBUG_CAMERA) PApplet.println("\t" + i);
      }
      ...
      // else
      // if (DEBUG_CAMERA) PApplet.println("Camera does not appear to provide data in a
      // format we can convert. Sorry.");
      if (DEBUG_CAMERA) PApplet.println("default imageformat:" + cameraParameters.getPreviewFormat());

      List<String> flashmodes = cameraParameters.getSupportedFlashModes();
      if (flashmodes != null && flashmodes.size() > 0) {
        for (String s : flashmodes)
          if (DEBUG_CAMERA) PApplet.println("supported flashmode: " + s);
        if (enableFlash)
          cameraParameters.setFlashMode(Parameters.FLASH_MODE_TORCH);
        else
          cameraParameters.setFlashMode(Parameters.FLASH_MODE_OFF);
      } else
        if (DEBUG_CAMERA) PApplet.println("No flash support.");
      ...
      if (info.facing == Camera.CameraInfo.CAMERA_FACING_FRONT) {
        if (DEBUG_CAMERA) PApplet.println("Front camera degrees="+degrees+" cameraRotationOffset="+cameraRotationOffset);
        requestedPortraitImage = true;
        //cameraRotationOffset += 180;
        result = (cameraRotationOffset + degrees) % 360;
        result = (360 - result) % 360; // compensate the mirror
      } else { // back-facing
        if (DEBUG_CAMERA) PApplet.println("Back camera degrees="+degrees+" cameraRotationOffset="+cameraRotationOffset);
        result = (cameraRotationOffset - degrees + 360) % 360;
      }
      camera.setDisplayOrientation(result);
      if (DEBUG_CAMERA) PApplet.println("Rotation reported: " + degrees);
      if (DEBUG_CAMERA) PApplet.println("camera: setting display orientation to: " + result + " degrees");
      camera.setDisplayOrientation(result);
      ...
      try {
        parent.getActivity().runOnUiThread(new Runnable() {
          public void run() {
            ...
            try {
              ...
              if (DEBUG_CAMERA) PApplet.println(
        ...
        camera.startPreview();
        if (DEBUG_CAMERA) PApplet.println("Something bad happened trying to open the preview: " + x.getMessage());
      }
      isStarted = true;
      ...
      if (DEBUG_CAMERA) PApplet.println("Using preview format: " + camera.getParameters().getPreviewFormat());

      if (DEBUG_CAMERA) PApplet.println("Preview size: " + frameWidth + "x" + frameHeight + "," + cameraFPS);
      if (DEBUG_CAMERA) PApplet.println("Photo size: " + photoWidth + "x" + photoHeight);
  ...
  //    savePhotoPath = "";
  //    return savePhoto(savePhotoPath, "_4x3.jpg");
  //  }
  //  return null;
  //}
   ...
   * @return true, if successful
   */
  public String savePhoto(String _filename, String filenamePrefix, String suffix) {
    photoSaved = true;
    String filename = "";
    ...
    if (_filename.startsWith(File.separator)) {
      ...
      if (!mediaStorageDir.exists()) {
        if (!mediaStorageDir.mkdirs()) {
          if (DEBUG_CAMERA) PApplet.println("failed to create directory to save photo: " + mediaStorageDir.getAbsolutePath());
          return null;
        }
      }
      savePhotoPath = mediaStorageDir.getAbsolutePath() + File.separator + filename;
    ...
    // test for file write, return false if error
    // otherwise call camera to save
    if (DEBUG_CAMERA) PApplet.println("Calculated photo path: " + savePhotoPath);
    ...
    //  File f = new File(savePhotoPath);
    //  if (!f.delete())
    //    if (DEBUG_CAMERA) PApplet.println("Failed to remove temp photoFile while testing permissions..oops");
    //}
    //catch (FileNotFoundException x) {
    //  if (DEBUG_CAMERA) PApplet.println("Failed to save photo to " + savePhotoPath + "\n" + x.getMessage());
    //  return null;
    //}
    //catch (IOException e) {
    //  if (DEBUG_CAMERA) PApplet.println("Failed to save photo to " + savePhotoPath + "\n" + e.getMessage());
  ...
  PreviewCallback previewcallback = new PreviewCallback() {
    public void onPreviewFrame(byte[] data, Camera camera) {
      ...
      try {
        ...
        if (DEBUG_CAMERA) PApplet.println(" onCameraPreviewEvent() had  an error:" + e.getMessage());
      ...
      if (onPreviewEventMethodPImage != null && myPixels != null) {
        try {
          ...
          if (DEBUG_CAMERA) PApplet.println(
            "Disabling onCameraPreviewEvent(KetaiCamera2) because of an error:" + e.getMessage());
          e.printStackTrace();
          onPreviewEventMethodPImage = null;
        }
      }
      ...
      // if (DEBUG_CAMERA) PApplet.println("Finding faces in preview using CV");
      ...
      // new Object[] { faces });
      // } catch (Exception e) {
      // if (DEBUG_CAMERA) PApplet.println("Exception trying to forward facedetection event
      // (KetaiCamera2):"
      // + e.getMessage());
      // }
      // }
  ...
  private AutoFocusCallback autofocusCB = new AutoFocusCallback() {
    public void onAutoFocus(boolean result, Camera c) {
      if (DEBUG_CAMERA) PApplet.println("Autofocus result: " + result);
    }
  };
  ...
  private PictureCallback jpegCallback = new PictureCallback() {
    public void onPictureTaken(byte[] data, Camera camera) {
      if (DEBUG_CAMERA) PApplet.println("pictureCallback entered...");
      if (camera == null)
        return;
      FileOutputStream outStream = null;

      try {
        if (DEBUG_CAMERA) PApplet.println("Saving image: " + savePhotoPath);
  ...
  private OnScanCompletedListener myScannerCallback = new OnScanCompletedListener() {
    public void onScanCompleted(String arg0, Uri arg1) {
      if (DEBUG_CAMERA) PApplet.println("Media Scanner returned: " + arg1.toString() + " => " + arg0);
    }
  };
   ...
   */
  public void stop() {
    if (DEBUG_CAMERA) PApplet.println("Stopping Camera...");
    requestedStart = false;
    if (camera != null && isStarted) {
      isStarted = false;
      camera.stopPreview();
  ...
  public Collection<? extends String> list() {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (DEBUG_CAMERA) PApplet.println("camera id[" + i + "] facing:" + facing);
    }
    return list;
  }
  ...
  @SuppressWarnings("deprecation")
    ...
    if (DEBUG_CAMERA) PApplet.println("Requested camera parameters as (w,h,fps):" + frameWidth + "," + frameHeight + "," + cameraFPS+" ar="+ar);
    // requested camera aspect ratio
    Parameters cameraParameters = camera.getParameters();
    // if (DEBUG_CAMERA) PApplet.println(cameraParameters.flatten());
    ...
    for (Size s : supportedSizes) {
      float ars = (float) s.width / (float) s.height;
      if (DEBUG_CAMERA) PApplet.println("Checking supported preview size:" + s.width + "," + s.height + " AR="+ars);
      if (nearestRequestedSize == null)
        nearestRequestedSize = s;

      if (!foundSupportedSize) {
        //if (s.width == frameWidth && s.height == frameHeight) {
        //  if (DEBUG_CAMERA) PApplet.println("Found matching camera size");
        //  nearestRequestedSize = s;
        //  foundSupportedSize = true;
        if (ar == ars) {
          if (DEBUG_CAMERA) PApplet.println("Found matching preview size "+ ar+ " "+ars);
          nearestRequestedSize = s;
          foundSupportedSize = true;
          break;
        } else {
    ...
    ar = (float)photoWidth/(float)photoHeight;
    for (Size s : supportedSizes) {
      float ars = (float) s.width / (float) s.height;
      if (DEBUG_CAMERA) PApplet.println("Checking supported picture size:" + s.width + "," + s.height + " AR="+ars);
      if (!foundSupportedSize) {
        if (ar == ars) {
          if (DEBUG_CAMERA) PApplet.println("Found matching picture size based on aspect ratio "+ ars);
          nearestRequestedSize = s;
          foundSupportedSize = true;
        }
        //} else if (s.width == photoWidth && s.height == photoHeight) {
        //  nearestRequestedSize = s;
    ...
    for (int r : supportedFPS) {
      if (DEBUG_CAMERA) PApplet.println("Supported preview FPS: " + r);
      if (nearestFPS == 0)
        nearestFPS = r;
    ...
    if (DEBUG_CAMERA) PApplet.println("calculated preview FPS: " + nearestFPS);

    cameraParameters.setPreviewFrameRate(nearestFPS);

    // if (DEBUG_CAMERA) PApplet.println("Setting calculated parameters:"
    ...
    if (cameraParameters.getMaxNumDetectedFaces() > 0) {
      if (DEBUG_CAMERA) PApplet.println("Face detection supported!");
  ...
  // new Object[] { faces });
  // } catch (Exception e) {
  // if (DEBUG_CAMERA) PApplet.println("Disabling onFaceDetectionEventMethod(KetaiCamera2)
  // because of an error:"
  // + e.getMessage());
  // e.printStackTrace();
  // onFaceDetectionEventMethod = null;
