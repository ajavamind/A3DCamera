...
public class Texture implements PConstants {
  ...
  public Texture(PGraphicsOpenGL pg, int width, int height, Object params) {
    this(pg, width, height, params, true);
  }


  /**
   * Creates an instance of PTexture with size width x height and with the
   * specified parameters. If clear is false, the opengl texture is allocated
   * to the given size, but its contents are not initialized, so no pixel
   * data is transfered to the GPU and the initial contents of the texture are
   * undefined.
   * @param width int
   * @param height int
   * @param params Parameters
  ...
  public Texture(PGraphicsOpenGL pg, int width, int height, Object params,
    ...
    init(width, height, (Parameters)params, clear);
  }
  ...
  public void init(int width, int height, Parameters params)  {
    init(width, height, params, true);
  }


  /**
   * Sets the size of the image and texture to width x height, and the
   * parameters of the texture to params. If clear is false, the texture
   * storage is allocated in the GPU but its contents are not initialized,
   * so no pixel data is transfered and the initial contents of the texture
   * are undefined.
   */
  public void init(int width, int height, Parameters params, boolean clear)  {
  ...
  /**
   * Allocates the opengl texture object. If clear is false, the pixel buffer
   * in video memory is not initialized with the clear color, so the contents
   * of the texture are undefined. This is useful when the texture is going
   * to be filled entirely with new data (for example, frames coming from a
   * camera) before it is drawn, since it avoids a costly initialization pass.
   */
  protected void allocate(boolean clear) {
