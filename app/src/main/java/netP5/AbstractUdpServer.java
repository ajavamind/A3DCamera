...
public abstract class AbstractUdpServer implements Runnable {
	...
	private volatile boolean isRunning = true;  // Andy Modla add volatile
	...
	public void start() {
		...
		// NOTE: removed the old Thread.sleep(1000) here. It blocked the
		// calling thread for a full second BEFORE the socket was bound, so
		// any broadcast sent within that window was silently lost.
		try {
		...
		// Only resolve the address when one was actually supplied. The
		// (listener, port) constructors leave _myAddress null, and
		// InetAddress.getByName(null) throws here.
		if (_myDatagramSocket != null && _myAddress != null) {
			try {
				_myInetAddress = InetAddress.getByName(_myAddress);
			} catch (UnknownHostException uhex) {
				Logger.printError("UdpServer.start()",
						" unknown host " + _myAddress + " " + uhex);
			}
		}
