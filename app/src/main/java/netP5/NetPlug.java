...
class NetPlug {
	...
	protected void checkMethod() {
		try {
			...
			// Do not swallow silently: a failure here means reflective
			// lookup of netEvent/netStatus failed and events will be lost.
			Logger.printError("NetPlug.checkMethod", "failed to locate event methods: " + e);
		}
	}
	...
	public void process(final DatagramPacket theDatagramPacket,
		...
		if (isNetListener || isEventMethod) {
			...
			// A packet arrived but nobody is registered to receive it.
			// Log it instead of dropping it silently.
			Logger.printWarning("NetPlug.process",
					"packet received on port " + thePort + " but no NetListener or netEvent(NetMessage) method is registered; message dropped.");
		}
	}
